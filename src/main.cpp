...
static void print_help() {
    ...
    cerr << "The EBNF definition of the program syntax:" << endl
         ...
         << "record  := 'r' [ append ] [ silence ] [ iter ] millis audiofile" << endl
         << "append  := 'a'" << endl
         << "silence := 's'" << endl
         << "closed  := 'c'" << endl
         << "iter    := 'i'" << endl
...
void TestProcess::Main() {
    ...
    initSignalHandling();
    manager = new Manager();
    if (manager->Init(args)) manager->Main(args);

    std::cout << "Exiting..." << std::endl;
    delete manager;
}

LocalEndPoint::LocalEndPoint(Manager &m) : OpalLocalEndPoint(m), m_manager(m) {
...
OpalLocalConnection *LocalEndPoint::CreateConnection(OpalCall &call, void *userData, unsigned opts,
...
void Manager::Main(PArgList &args) {
    ...
    if (!Command::Parse(cmdseq, sequence)) {
        cerr << "Problem parsing command string \"" << args.GetOptionString('x') << "\":" << endl
    ...
    // run it
        ...
        cerr << "Problem running command sequence (\"" << args.GetOptionString('x') << "\"):" << endl
             << "  " << Command::GetErrorString() << endl;
...
bool Manager::Init(PArgList &args) {
    ...
    if (args.HasOption('o')) PTrace::Initialise(5, args.GetOptionString('o'));
    ...
    if (args.HasOption('p')) {
        TPState::Instance().SetListenPort(args.GetOptionString('p').AsInteger());
    }
    ...
    if (!protocol.compare("sip")) {
        ...
        if (args.HasOption('c')) {
            ...
            if (!sipep->Register(param, *aor)) {
                cout << "Could not register to " << param.m_registrarAddress << endl;
                return false;
            } else {
                cout << "registered as " << aor->GetPointer(aor->GetSize()) << endl;
            }
        }
...
bool Manager::SendDTMF(const PString &dtmf) {
    ...
    if (!call) {
        std::cerr << "no call found with token=" << currentCallToken << std::endl;
        return false;
    }
...
void RTPSession::SelectAudioFormat(const Payload payload) {
    if (m_audioformat) delete m_audioformat;

    switch (payload) {
        case PCM16:
            m_audioformat =
                new OpalAudioFormat("OPAL_PCM16", RTP_DataFrame::MaxPayloadType, "", 16, 8, 240, 0, 256, 8000, 0);
            cout << "Payload format: OPAL_PCM16" << endl;
            break;
        case G711_ULAW:
            m_audioformat =
                new OpalAudioFormat("OPAL_G711_ULAW_64K", RTP_DataFrame::PCMU, "PCMU", 16, 8, 240, 0, 256, 8000, 0);
            cout << "Payload format: OPAL_G711_ULAW_64K" << endl;
            break;
        case G711_ALAW:
            m_audioformat =
                new OpalAudioFormat("OPAL_G711_ALAW_64K", RTP_DataFrame::PCMA, "PCMA", 16, 8, 240, 0, 256, 8000, 0);
            cout << "Payload format: OPAL_G711_ULAW_64K" << endl;
            break;
    }
}
...
RTP_Session::SendReceiveStatus RTPSession::OnReceiveData(RTP_DataFrame &frame) {
    ...
    TPState::Instance().GetRecordAudio().RecordFromBuffer((const char *)frame.GetPayloadPtr(), frame.GetPayloadSize(),
                                                          true);
    return ret;
}
...
bool Manager::MakeCall(const PString &remoteParty) {
    ...
    if (TPState::Instance().GetProtocol() != TPState::RTP) {
        ...
        if (!m_rtpsession->Open(local, TPState::Instance().GetListenPort(), TPState::Instance().GetListenPort(), 2)) {
            cerr << "could not open rtp session" << endl;
            return false;
        }

        m_rtpsession->SetJitterBufferSize(100, 1000);
        std::cout << "RTP local address:     " << local << std::endl
...
unsigned Manager::CalculateTimestamp(const size_t size) {
    ...
    if (framesize == 0) return frametime;

    unsigned frames = (size + framesize - 1) / framesize;
    return frames * frametime;
}

bool Manager::WriteFrame(RTP_DataFrame &frame) { return m_rtpsession && m_rtpsession->Internal_WriteData(frame); }

bool Manager::ReadFrame(RTP_DataFrame &frame) { return m_rtpsession && m_rtpsession->ReadBufferedData(frame); }

bool Manager::StartListener() {
    // TODO h323.
    PIPSocket::Address sipaddr = INADDR_ANY;
    cout << "Listening for SIP signalling on " << sipaddr << ":" << TPState::Instance().GetListenPort() << endl;
...
bool Manager::OnOpenMediaStream(OpalConnection &connection, OpalMediaStream &stream) {
    std::cout << __func__ << std::endl;
    ...
    std::cout << (stream.IsSink() ? "streaming media to " : "recording media from ") << prefix << std::endl;
...
void Manager::OnEstablishedCall(OpalCall &call) {
    ...
    TPState::Instance().SetState(TPState::ESTABLISHED);
    currentCallToken = std::string(static_cast<const char *>(call.GetToken()));

    std::cout << "In call with " << call.GetPartyB() << " using " << call.GetPartyA() << " token=[" << currentCallToken
              << "]" << std::endl;
...
void Manager::OnReleased(OpalConnection &connection) {
    ...
    std::cout << __func__ << ": reason: " << get_call_end_reason_string(r) << std::endl;
